...
function Worker(queues, options) {
    ...
    this.errorHandler = options.errorHandler || function noop(){};
...
Worker.prototype.poll = function () {
    ...
    this.dequeue(function (err, job) {
        if (err) {
            if (err.message.indexOf('collection going away') !== -1) {
                console.log('handling \'collection going away\' error');
                return self.poll();
            }
...
Worker.prototype.work = function (job) {
    ...
    function done(err, result) {
        ...
        profiler.endAll();
        job.data.stats = profiler.getStats();
    ...
    this.process(profiler, job.data, done);
};

Worker.prototype.process = function (profiler, data, callback) {
    var func = this.callbacks[data.name];

    if (!func) {
        callback(new Error('No callback registered for `' + data.name + '`'));
    } else {
        func.call(profiler, data.params, callback);
    }
};
...
Worker.prototype.error = function (job, err, callback) {
    ...
    if (remaining > 0) {
        ...
        if (attempts.delay !== undefined) {
            wait = strategy(attempts, err, job.data);
        }
        if (wait === null) {
